...
const LeaveEdit = () => {
    ...
    useEffect(()=>{
        ...
        axios.get(`https://www.auto.fanwebco.com/InOut_api/api/LeaveManagementController/EditLeave?leaveid=${leaveId}`,{headers})
        .then(res => {setLoading(false);setLeaveData(res.data.leaveViewModel);setError(false)})
        .catch(error => {setLoading(false);setLeaveData([]);setError(error)})
    },[])
    ...
    const{leaveRequestDate,leaveRequestOutcome,leaveRequestConfirmDate,adminDescription,isAdminRead,userID_Request,userID_Request_UserName,userID_Request_FullName,userID_Confirm,userID_Confirm_UserName,userID_Confirm_FullName,...leaveDataa}=leaveData
        ...
        if(!leaveDataa.fromDate_Day || !leaveDataa.toDate_Day){
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                  axios.put('https://www.auto.fanwebco.com/InOut_api/api/LeaveManagementController/EditLeave',leaveDataa,{headers:headers})
                  .then(res =>{Swal.fire({
                      title:`${res.data.msg}`,
                      icon:"success",
    ...
    const deleteHandler=()=>{
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                axios.delete(`https://www.auto.fanwebco.com/InOut_api/api/LeaveManagementController/DeletLeave?leaverequestid=${leaveData.id}`,{headers:headers})
                .then(res =>{Swal.fire({
                    title:`${res.data.msg}`,
                    icon:"success",
                    timerProgressBar:"true"
    ...
    return (
        <div>
            ...
            <div className='formRequestLeave mt-32 mx-5 sm:mx-10 bg-zinc-100 px-10 py-5  border rounded-lg  transition-all '>
                  <p className='my-3'>نام و نام خانوادگی : {leaveData.userID_Request_FullName}  </p>
                  <div>
                     <select onChange={changeLeaveTypeHandler} value={leaveData.leaveType} className='w-full border py-2 my-2 rounded-md px-3 border-zinc-500 bg-purple-100' name="" id="">
                  ...
                  <div className='my-3'>
                    <textarea onChange={descriptionHandler} value={leaveData.description}  className=' w-full border rounded-md bg-purple-100 p-2' placeholder='توضیحات...' name="" id="" cols="30" rows="10"></textarea>
                  </div>
                  <div className='grid grid-cols-2 md:grid-cols-2 gap-5'>
                    <button onClick={editHandler} className='text-white w-full border p-2 rounded-xl bg-green-400 flex items-center justify-center '>ویرایش مرخصی</button>
                    <button onClick={deleteHandler} className='text-white w-full border p-2 rounded-xl bg-red-400 flex items-center justify-center '>حذف مرخصی</button>
                  </div>
            </div>
        </div>
    );
