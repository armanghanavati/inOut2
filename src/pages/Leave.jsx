...
const Leave = () => {
    ...
    useEffect(() => {
        ...
        axios.get('https://www.auto.fanwebco.com/InOut_api/api/LeaveManagementController/AllUserLeavesRequeset', { headers })
            .then(res => { setLoading(false); setLeaves(res.data.leaveRequestsList ? res.data.leaveRequestsList : []); setError(false) })
            .catch(error => { setLoading(false); setLeaves([]); setError(error) })
    }, [leaves])
    ...
    const addLeaveHandler = () => {
        ...
        if (!addleaveData.fromDate_Day || !addleaveData.toDate_Day) {
            ...
            axios.post('https://www.auto.fanwebco.com/InOut_api/api/LeaveManagementController/CreateLeave', addleaveData, { headers: headers })
                .then(res => {
                    Swal.fire({
                        title: `${res.data.msg}`,
                        icon: "success",
                        timerProgressBar: "true"
                    }); setOpen(false); resetformRequestLeave()
                })
        ...
        <div>
            ...
            <div className='mt-40 mb-8'>
                <div className=' grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 mx-5 mt-24 mb-36'>
                    ...
                    ))) : (<div className='border bg-red-200 p-10 font-bold text-red-500'>هیچ درخواست مرخصی ثبت نشده است</div>)}
            ...
            <div className={'formRequestLeave z-[6] bg-zinc-100 px-10 py-5 fixed top-36 bottom-14 overflow-y-auto border rounded-lg right-14 left-14 transition-all ' + (open ? ' visible opacity-100 ' : ' invisible opactiy-0')}>
                <div onClick={() => { setOpen(false); resetformRequestLeave() }} className='fixed text-red-500 top-32 right-10 transition-all'>
                    <FaRegCircleXmark className='bg-red-500 rounded-full' size='40px' color='white' />
                </div>
                <p className='my-3'>{userData.firstName} {userData.family} </p>
                <div>
                    <select onChange={changeleaveTypeHandler} value={addleaveData.leaveType} className='w-full border py-2 my-2 rounded-md px-2 border-zinc-500 bg-purple-100' id="">
                ...
                <div className='my-3'>
                    <textarea onChange={descriptionHandler} value={addleaveData.description} className=' w-full border rounded-md bg-purple-100 p-2' placeholder='توضیحات...' name="" id="" cols="30" rows="10"></textarea>
                </div>
                <div>
                    <button onClick={addLeaveHandler} className=' w-full border p-2 rounded-xl bg-purple-400 flex items-center justify-center'>ثبت مرخصی</button>
                </div>
            </div>
